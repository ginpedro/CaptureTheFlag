...
enum {target_found, target_not_found, search_incomplete, item_found, item_not_found};
...
public:
  ...
  { 
   ...
   ~Graph_SearchAStar_TS(){delete m_pPQ; delete heuristic;} //NEW: deletar o ponteiro para Heuristic
...
//  update-steps
//-----------------------------------------------------------------------------
template <class trigger_type, class graph_type> //NEW: classe alterada, antes era um template <class graph_type, class termination_condition>.
...
private:
  ...
  TerminatePolicy<trigger_type,graph_type>*        tCon;
  const graph_type&                   m_Graph;
...
public:
  ...
  Graph_SearchDijkstras_TS(const graph_type&  G,
                          int                   source,
                          int                   target,
						  TerminatePolicy<trigger_type,graph_type>* t_con):Graph_SearchTimeSliced<Edge>(Dijkstra),
  ...
  //to the termination condition)
   ...
   {
     delete m_pPQ;
	 delete tCon; //NEW: deletar ponteiro para TerminatePolicy
   }
...
int Graph_SearchDijkstras_TS<trigger_type,graph_type>::CycleOnce()
{
  //if the PQ is empty the target has not been found
  if (m_pPQ->empty())
  {
  ...
  //if the target has been found exit
  if (tCon->isSatisfied(m_Graph, m_iTarget, NextClosestNode)) //NEW: originalmente onde estao 'tCon->' era um 'terminal_condition::'
  {
...
template <class trigger_type, class graph_type>
std::list<int> 
Graph_SearchDijkstras_TS<trigger_type,graph_type>::GetPathToTarget()const
{
  std::list<int> path;
...
template <class trigger_type, class graph_type>
std::list<PathEdge> 
Graph_SearchDijkstras_TS<trigger_type,graph_type>::GetPathAsPathEdges()const
{
  std::list<PathEdge> path;
