...
{    
  ...
  typedef FindActiveTrigger<Trigger<Raven_Bot>,Raven_Map::NavGraph> t_con; 
...
bool Raven_PathPlanner::RequestPathToPositionAndAvoid(Vector2D TargetPos, double MaxDistFromEnemy) {
  ...
  //be calculated, the bot can go straight to the position by ARRIVING at
  //the current waypoint
  if (m_pOwner->canWalkTo(TargetPos) && m_pOwner->GetSensoryMem()->GetListOfRecentlySensedOpponents().empty())
  { 
    return true;
  }
...
//------------------------------- RequestPathToItemSecure -------------------------------
//
//---------------------------------------------------------------------------------------
bool Raven_PathPlanner::RequestPathToItemSecure(unsigned int ItemType, double MaxDistFromEnemy)
{
	//clear the waypoint list and delete any active search
  GetReadyForNewSearch();
...
//------------------------------- RequestPathToSecurePlace ------------------------------
//
//---------------------------------------------------------------------------------------
 bool Raven_PathPlanner::RequestPathToSecurePlace(double MaxDistFromEnemy) {
	 //clear the waypoint list and delete any active search
  GetReadyForNewSearch();
